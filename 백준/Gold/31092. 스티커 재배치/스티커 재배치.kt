...
class `스티커 재배치` {
    ...
    fun solve() {
        ...
        val tmpForRemove = mutableListOf<Pair<Char, Int>>()
        ...
        repeat(m) { id ->
            ...
            tmpForRemove.add(s.first() to d.toInt())
        }
//        println(outOfAlpha)
//        println(outOfAlpha)
//        println(cheapestPrice)
        ...
        board.forEach { idx ->
            val (alpha, outPrice) = tmpForRemove[idx]
            outOfAlpha[alpha]?.add(outPrice) ?: run { outOfAlpha[alpha] = mutableListOf(outPrice) }
        }
        val s = readln()
...
//            println(outOfAlphaInRange)
            for (j in i until i + s.length) {
                val (alpha, out) = tmpForRemove[board[j]]
                outOfAlphaInRange[alpha]!!.remove(out)
            }
            var price = 0
...
//                        println("$i, $j, $price, 34343")
                        ...
                        if ((outOfAlphaInRange[findSticker]?.minOrNull() ?: Int.MAX_VALUE) < priceOfAlpha[findSticker]!!) {
